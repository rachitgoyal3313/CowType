...
function updateCursorPosition() {
    ...
    if (currentWord && currentLetter) {
        ...
        const currentMarginTop = parseInt(words.style.marginTop || '0');
        
        // Calculate cursor position
        const cursorTop = letterRect.top - wordsRect.top + 
            (currentMarginTop * -1) + 
            (letterRect.height - cursor.offsetHeight) / 2;
        
        const cursorLeft = letterRect.left - wordsRect.left - cursor.offsetWidth / 2;
        
        // Set cursor position
        cursor.style.top = `${Math.max(0, cursorTop)}px`;
        cursor.style.left = `${Math.max(0, cursorLeft)}px`;
    }
}
